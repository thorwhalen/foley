...
**never** a [`Candidate`](foley.base.md#foley.base.Candidate), a [`WeaveResult`](foley.md#foley.WeaveResult), or numpy
...
surfaces: Python / CLI / MCP): they resolve the shared [`foley.index.SoundLibrary`](foley.index.md#foley.index.SoundLibrary)
and per-session [`foley.agent.session.SessionStore`](foley.agent.session.md#foley.agent.session.SessionStore) (both injectable for tests).
...
onto its candidate’s [`SoundEvent`](foley.base.md#foley.base.SoundEvent) so `plan` honors the agent’s
