...
resolve a registered live [`SourceAdapter`](foley.sources.base.md#foley.sources.base.SourceAdapter), search it
(with the license filter pushed into the query), gate each hit through the
fail-closed [`foley.keep()`](foley.md#foley.keep) license check, fetch its (transient) bytes, and run
them through the SAME [`foley.index.ingest.ingest_one()`](foley.index.ingest.md#foley.index.ingest.ingest_one) pipeline the local and
