...
into [`foley.agent.find()`](foley.agent.md#foley.agent.find) by keyword (`decomposer=` / `judge=` / `refiner=`),
...
[`foley.index.protocols`](foley.index.protocols.md#module-foley.index.protocols)), so importing this module pulls no heavy dependency and
