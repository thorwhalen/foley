...
[`OTelTracer`](foley.obs.trace.md#foley.obs.trace.OTelTracer) only, so importing this package is dol-only.
...
|-------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------|
| [`ingest_digest`](#foley.obs.ingest_digest)(report)                                | A leak-free digest of an [`IngestReport`](foley.index.ingest.md#foley.index.ingest.IngestReport) for a manifest. |
...
* **Parameters:**
  * **store** ([`MutableMapping`](https://docs.python.org/3/library/typing.html#typing.MutableMapping)) – A `MutableMapping[str, dict]` (default:
    [`foley.stores.make_run_store()`](foley.stores.md#foley.stores.make_run_store); a dict in tests).
  * **manifest** ([`RunManifest`](foley.obs.run_artifact.md#foley.obs.run_artifact.RunManifest)) – The [`RunManifest`](#foley.obs.RunManifest) to persist.
  * **redactor** – An optional [`Redactor`](foley.obs.redact.md#foley.obs.redact.Redactor) applied to the full
...
* **Return type:**
  [`Tracer`](foley.obs.trace.md#foley.obs.trace.Tracer)
* **Returns:**
  A [`Tracer`](#foley.obs.Tracer) (never raises).

### foley.obs.ingest_digest(report)

A leak-free digest of an [`IngestReport`](foley.index.ingest.md#foley.index.ingest.IngestReport) for a manifest.
...
`force_disabled` (set by [`foley.runtime.offline_scope()`](foley.runtime.md#foley.runtime.offline_scope) for a telemetry-off
...
* **Return type:**
  [`RunManifest`](foley.obs.run_artifact.md#foley.obs.run_artifact.RunManifest)
...
* **Parameters:**
  ...
  * **mode** ([`RedactionMode`](foley.obs.redact.md#foley.obs.redact.RedactionMode)) – `off` → `None`; `full` → `text` verbatim; `hash` (default) →
...
hard-off `force_disabled` flag: [`foley.runtime.offline_scope()`](foley.runtime.md#foley.runtime.offline_scope) sets it for a
...
| [`recorder`](foley.obs.recorder.md#module-foley.obs.recorder)         | The run-recorder: owns the manifest, drives the OTel mirror, off by default (#11).   |
|---------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------|
| [`redact`](foley.obs.redact.md#module-foley.obs.redact)             | Redaction of sensitive narration / prompt / query text in telemetry (#11).           |
| [`run_artifact`](foley.obs.run_artifact.md#module-foley.obs.run_artifact) | The reproducible run-artifact SSOT — `RunManifest` + `SpanRecord` (#11).             |
| [`trace`](foley.obs.trace.md#module-foley.obs.trace)               | The OpenTelemetry MIRROR seam for foley's observability (#11).                       |
