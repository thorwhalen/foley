...
[`foley.sources.registry`](foley.sources.registry.md#module-foley.sources.registry) and *wrap* the ingest machinery, never fork it.

### Functions

| [`bulk_license`](#foley.sources.bulk_license)(\*, source, license_id, ...[, ...])   | Build a bulk-acquisition [`LicenseRecord`](foley.base.md#foley.base.LicenseRecord), flags derived.                   |
|-----------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------|
...
| [`api_license`](#foley.sources.api_license)(\*, source, license_id, ...[, ...])    | Build an API-acquisition [`LicenseRecord`](foley.base.md#foley.base.LicenseRecord), flags derived.                   |
...
| [`generated_license`](#foley.sources.generated_license)(\*, source, license_id, ...)     | Build an AI-generated [`LicenseRecord`](foley.base.md#foley.base.LicenseRecord), flags derived + provenance stamped. |
| [`generate`](#foley.sources.generate)(prompt, \*[, backend, library, ...])      | Generate a sound via `backend` and ingest it (by-value) into `library`.                                                                            |
| [`candidate_of`](#foley.sources.candidate_of)(result)                               | Wrap a stored [`IngestResult`](foley.index.ingest.md#foley.index.ingest.IngestResult) as a generated candidate.              |
...
| [`GenerationError`](#foley.sources.GenerationError)(message, \*, report, status)     | Raised by [`foley.generate()`](foley.md#foley.generate) when a backend yields no stored sound.        |
|---------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------|
...
[`LicenseRecord`](foley.base.md#foley.base.LicenseRecord), and returns a [`GeneratedClip`](#foley.sources.GeneratedClip). The
...
report; only the public [`foley.generate()`](foley.md#foley.generate) promise raises.
...
[`GenerationError`](#foley.sources.GenerationError) so the public [`foley.generate()`](foley.md#foley.generate) `Raises` clause
...
* **Returns:**
  ...
  `.summary()` for counts, exactly like [`foley.ingest()`](foley.md#foley.ingest).
...
routed through the shared [`ingest_one()`](foley.index.ingest.md#foley.index.ingest.ingest_one) pipeline
...
* **Safety gate** — the prompt is scanned for trademarked audio logos + cloned
  voices *before* any synthesis spend ([`foley.provenance.disclosure.scan_prompt()`](foley.provenance.disclosure.md#foley.provenance.disclosure.scan_prompt)).
  ...
  `contains_recognizable_voice` on the record so [`foley.keep()`](foley.md#foley.keep) drops it
  downstream.
...
* **Raises:**
  ...
  * [**WatermarkUnavailable**](foley.provenance.disclosure.md#foley.provenance.disclosure.WatermarkUnavailable) – If `watermark=True` but `foley[provenance]` is absent.
...
| [`base`](foley.sources.base.md#module-foley.sources.base)                 | The bulk-corpus source contract — a downloaded corpus as an ingestable stream.            |
|-------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------|
| [`bbc_remarc`](foley.sources.bbc_remarc.md#module-foley.sources.bbc_remarc)     | BBC RemArc (Rewind Archive) — a Ring-2, quarantined corpus.                               |
| [`clotho`](foley.sources.clotho.md#module-foley.sources.clotho)             | Clotho-eval — a captioned Ring-0 corpus that doubles as a retrieval fixture.              |
| [`elevenlabs`](foley.sources.elevenlabs.md#module-foley.sources.elevenlabs)     | ElevenLabs Sound Effects generate source (hosted; #6).                                    |
| [`foleyset`](foley.sources.foleyset.md#module-foley.sources.foleyset)         | FoleySet — a CC-BY, Foley-native starter corpus (Ring 0).                                 |
| [`freesound`](foley.sources.freesound.md#module-foley.sources.freesound)       | Freesound APIv2 retrieve source — CC0 sounds, stored by-reference (`foley[freesound]`).   |
| [`fsd50k`](foley.sources.fsd50k.md#module-foley.sources.fsd50k)             | FSD50K — the labeled, commercial-safe backbone corpus (Ring 1).                           |
| [`http`](foley.sources.http.md#module-foley.sources.http)                 | Injectable HTTP transport for foley's live source adapters.                               |
| [`pull`](foley.sources.pull.md#module-foley.sources.pull)                 | `add_from` — the live-source pull façade (SOURCE → INDEX in one call).                    |
| [`registry`](foley.sources.registry.md#module-foley.sources.registry)         | Live-source adapter registry: auto-discovery + lazy loading (arioso's registry, ported).  |
| [`resilience`](foley.sources.resilience.md#module-foley.sources.resilience)     | Graceful degradation for HTTP source adapters — throttle · backoff · circuit-break (#12). |
| [`sonniss`](foley.sources.sonniss.md#module-foley.sources.sonniss)           | Sonniss GameAudioGDC — a Ring-2, quarantined corpus.                                      |
| [`stable_audio`](foley.sources.stable_audio.md#module-foley.sources.stable_audio) | Stable Audio Open 1.0 generate source (local default; #6).                                |
